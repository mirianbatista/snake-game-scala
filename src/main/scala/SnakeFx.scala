...
object SnakeFx extends JFXApp3 {
  ...
  case class State(snake: List[(Double, Double)], food: (Double, Double)) {
    ...
    def rectangles: List[Rectangle] = square(food._1, food._2, Green) :: snake.map {
      case (x, y) => square(x, y, Color.web("#5B7AFA"))
    }
  }
    ...
    stage = new JFXApp3.PrimaryStage {
      ...
      scene = new Scene {
        ...
        onKeyPressed = key => key.getCode match {
          ...
          case _ => // Faça nada para outras teclas
        }
